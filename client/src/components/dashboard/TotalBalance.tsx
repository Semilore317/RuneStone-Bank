    ...
    <Card className="border-t-8 border-t-ochre-800 bg-zinc-900 border-x-4 border-b-4 border-x-zinc-800 border-b-zinc-800 text-white relative overflow-hidden group/card">
      ...
      <CardHeader className="border-b-4 border-zinc-800 pb-2">
        <CardTitle className="text-zinc-400 text-sm md:text-base flex items-center justify-between">
          ...
          <button
            onClick={loadBalance}
            className={`p-1 hover:text-white transition-colors ml-2 ${isLoading ? "animate-spin" : ""}`}
          >
            <RefreshCw size={16} />
          </button>
        </CardTitle>
      ...
      <CardContent className="pt-6">
        <div
          className={`text-4xl md:text-6xl font-black break-words tracking-tighter transition-all duration-300 ${!isRevealed ? "blur-md select-none opacity-50" : ""}`}
        >
          $
          {balanceInfo.accountBalance.toLocaleString(undefined, {

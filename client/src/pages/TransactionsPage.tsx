...
export function TransactionsPage() {
  ...
  const handleRequestStatement = async (startDate: string, endDate: string) => {
    if (!user?.accountNumber) return;
    try {
      const response = await requestStatement(
        user.accountNumber,
        startDate,
        endDate,
      );
  ...
  const filtered = transactions.filter((tx) => {
    const matchesSearch =
      ...
      (tx.counterpartyName?.toLowerCase() || "").includes(
  ...
  const totalOut = transactions
    .filter(
      (t) =>
        (t.transactionType === "DEBIT" || t.transactionType === "TRANSFER") &&
        t.status === "SUCCESS",
    )
    .reduce((s, t) => s + t.amount, 0);
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        ...
        >
          {(["ALL", "CREDIT", "DEBIT", "TRANSFER"] as FilterType[]).map(
            (type) => (
              <button
                key={type}
  ...
  return (
    <div className="py-5 flex items-center justify-between group hover:bg-zinc-800/30 transition-colors px-2">
      <div className="flex items-center gap-4">
        <div
          className={`p-3 border-2 border-zinc-800 bg-zinc-950 ${isPositive ? "text-green-400" : "text-red-500"}`}
        >
          {isPositive ? (
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-xs font-bold uppercase px-2 py-0.5 border-2 ${
      ...
      <div className="text-right">
        ...
        <p className="text-zinc-600 font-mono text-xs mt-1">
          {transaction.counterpartyAccountNumber || transaction.accountNumber}
        </p>
      </div>
    </div>
  );
